...
var (
	ErrEtcdMultipleBootstrap = errors.New("client setup failed: multiple discovery or bootstrap flags specified, use either \"address\" or \"discovery_srv\"")
	ErrEtcdAddress           = errors.New("client setup failed: address must be valid URL (ex. 'scheme://host:port')")
	ErrEtcdLockHeld          = errors.New("lock already held")
	ErrEtcdLockNotHeld       = errors.New("lock not held")
	ErrEtcdVersionUnknown    = errors.New("etcd: unknown API version")
)
...
func NewEtcdBackend(conf map[string]string, logger log.Logger) (physical.Backend, error) {
	...
	default:
		return nil, ErrEtcdVersionUnknown
	}
}

// getEtcdOption retrieves the config option in order of priority:
//  1. The named environment variable, if it exists.
...
func getEtcdEndpoints(conf map[string]string) ([]string, error) {
	...
	if useSrv && staticBootstrap {
		return nil, ErrEtcdMultipleBootstrap
	}
	...
	if useSrv {
		...
		srvClients, err := srv.GetClient("etcd-client", domain, srvName)
		if err != nil {
			return nil, fmt.Errorf("failed to discover etcd endpoints through SRV discovery: %w", err)
		}
		return srvClients.Endpoints, nil
	}
