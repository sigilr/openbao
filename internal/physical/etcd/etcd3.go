...
// EtcdBackend is a physical backend that stores data at a specific prefix
// within etcd. It is used for most production situations, as it allows
// OpenBao to run on multiple machines in a highly-available manner.
...
// newEtcd3Backend constructs an etcd3 backend.
func newEtcd3Backend(conf map[string]string, logger log.Logger) (physical.Backend, error) {
	// Get the etcd path from the configuration.
	etcdPath, ok := conf["path"]
	if !ok {
		etcdPath = "/vault"
	}

	// Ensure the path is prefixed.
	if !strings.HasPrefix(etcdPath, "/") {
		etcdPath = "/" + etcdPath
	}
	...
	if sReqTimeout == "" {
		// etcd3's default request timeout is 5s. That should be long enough for most
		// cases, even with internal retry.
		sReqTimeout = "5s"
	}
	...
	if sLock == "" {
		// etcd3's default lease duration is 60s; set to 15s for faster recovery.
		sLock = "15s"
	}
...
// component, with a trailing slash for "folders"), relying on etcd's
// lexicographically-sorted Get response order. It is shared by List and
// ListPage.
func (c *EtcdBackend) listAll(ctx context.Context, prefix string) ([]string, error) {
	...
	ctx, cancel := context.WithTimeout(ctx, c.requestTimeout)
	defer cancel()
	fullPrefix := path.Join(c.path, prefix) + "/"
	resp, err := c.etcd.Get(ctx, fullPrefix, clientv3.WithPrefix(), clientv3.WithKeysOnly())
	if err != nil {
		return nil, err
	}
	...
	for _, kv := range resp.Kvs {
		key := strings.TrimPrefix(string(kv.Key), fullPrefix)
...
func (c *EtcdBackend) ListPage(ctx context.Context, prefix, after string, limit int) ([]string, error) {
...
func (c *EtcdBackend) HAEnabled() bool {
	return c.haEnabled
}

// EtcdLock implements a lock using an etcd backend.
