...
import (
	...
	azuritehelper "github.com/openbao/openbao/v2/internal/helper/testhelpers/azurite"
)

// These tests run against an Azurite docker container, unless
// AZURE_ACCOUNT_NAME is given.
//
// Authentication options:
//   - Use a static access key via AZURE_ACCOUNT_KEY
//   - Use managed identities (leave AZURE_ACCOUNT_KEY empty)
//
// To run the tests using managed identities, the following pre-requisites
// have to be met:
//  1. Access to the Azure Instance Metadata Service (IMDS) is required
//     (e.g. run it on an Azure VM)
//  2. A system-assigned or user-assigned identity attached to the host
//     running the test
//  3. A role assignment for a storage account with "Storage Blob Data
...
func testFixture(t *testing.T) (*AzureBackend, func()) {
	...
	name := fmt.Sprintf("openbao-test-%d", ts)
	...
	if os.Getenv("AZURE_ACCOUNT_NAME") == "" {
		dockerCleanup, cfg := azuritehelper.PrepareTestContainer(t, "")
		backendConf["accountName"] = cfg.AccountName
		backendConf["accountKey"] = cfg.AccountKey
		backendConf["testHost"] = cfg.Endpoint
	...
	return azBackend, func() {
		if _, err := azBackend.container.GetProperties(context.Background(), nil); err != nil {
			t.Logf("failed to retrieve blob container info: %v", err)
			return
		}

		if _, err := azBackend.container.Delete(context.Background(), nil); err != nil {
...
func TestAzureBackend_ListPaging(t *testing.T) {
	...
	for i := range MaxListResults + 100 {
