...
func (l *Lock) renewLock() {
	...
	for {
		select {
		case <-ticker.C:
			if _, err := l.writeLock(); err != nil {
				l.backend.logger.Warn("failed to renew lock", "error", err)
			}
		case <-l.stopCh:
			return
		}
...
OUTER:
	for {
		...
		if (lockRecordCache == nil) ||
			...
			(time.Since(lockRecordCache.lastUpdate) > l.ttl) {
...
func (l *Lock) writeLock() (bool, error) {
	...
	if (lockRecordCache == nil) || lockRecordCache.lockRecord == nil ||
		...
		if currentLockRecord != nil && time.Since(lockRecordCache.lastUpdate) < LockCacheMinAcceptableAge {
			return false, nil
		}
	...
	l.backend.logger.Debug("Lock written", "record", string(newLockRecordJSON))
