...
//
// Note: the original Vault implementation of this backend also satisfied
// the (now removed) one-shot physical.Transactional interface via a
// Transaction(ctx, []*physical.TxnEntry) method. That interface no longer
// exists in this SDK; it has been replaced by the interactive
// physical.TransactionalBackend (BeginTx/BeginReadOnlyTx) contract. FDB's
// fdb.Database.CreateTransaction() could plausibly back that interface
// without the goroutine-relay tricks that made this infeasible for
// CockroachDB and Google Cloud Spanner (see those backends' source for
// details), but since this backend requires a CGo build tag and the native
// libfdb_c client library, it cannot be build- or test-verified in a
// standard (CGO_ENABLED=0, no native FDB client) environment. We
// deliberately do not add new, unverified transactional code here; this
// mirrors the conservative choice made for CockroachDB and Spanner.
var (
...
// listAll lists all the keys under a given prefix, up to the next prefix,
// in sorted order. Returns an empty slice for non-existent directories.
func (f *FDBBackend) listAll(ctx context.Context, prefix string) ([]string, error) {
	...
	pathRange := fdb.KeyRange{Begin: rangeBegin, End: rangeEnd}
...
func (f *FDBBackend) List(ctx context.Context, prefix string) ([]string, error) {
	...
	return f.listAll(ctx, prefix)
}

// ListPage is used to list a page of keys under a given prefix, in sorted
// order, starting after the given key. FDB's range reads are already
// returned in lexicographic key order, so we page over the full listing
// client-side rather than issuing a second range query.
func (f *FDBBackend) ListPage(ctx context.Context, prefix string, after string, limit int) ([]string, error) {
	defer metrics.MeasureSince([]string{"foundationdb", "list_page"}, time.Now())

	keys, err := f.listAll(ctx, prefix)
	if err != nil {
		return nil, err
	}
