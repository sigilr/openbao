...
func NewAliCloudOSSBackend(conf map[string]string, logger log.Logger) (physical.Backend, error) {
	...
	if _, err := bucketObj.ListObjects(); err != nil {
...
func (a *AliCloudOSSBackend) Put(_ context.Context, entry *physical.Entry) error {
...
func (a *AliCloudOSSBackend) Get(_ context.Context, key string) (*physical.Entry, error) {
	...
	if err != nil {
		...
		if errors.As(err, &svcErr) && svcErr.StatusCode == http.StatusNotFound && svcErr.Code == "NoSuchKey" {
			return nil, nil
		}
		return nil, err
	}
...
func (a *AliCloudOSSBackend) Delete(_ context.Context, key string) error {
...
// listAll returns the sorted set of "directory" entries and object keys
// directly under prefix.
func (a *AliCloudOSSBackend) listAll(prefix string) ([]string, error) {
	keys := []string{}
...
func (a *AliCloudOSSBackend) List(_ context.Context, prefix string) ([]string, error) {
...
func (a *AliCloudOSSBackend) ListPage(_ context.Context, prefix string, after string, limit int) ([]string, error) {
	defer metrics.MeasureSince([]string{AlibabaMetricKey, "list_page"}, time.Now())
