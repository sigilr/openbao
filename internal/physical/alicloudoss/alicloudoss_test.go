...
// TestAliCloudOSSBackend is an acceptance test gated on real Alicloud OSS
// credentials; there is no local OSS emulator this could run against
// instead.
func TestAliCloudOSSBackend(t *testing.T) {
	...
	bucket := fmt.Sprintf("openbao-alicloud-testacc-%d", randInt)

	if err := conn.CreateBucket(bucket); err != nil {
	...
	defer func() {
		...
		if _, err := b.DeleteObjects(objects); err != nil {
			t.Fatalf("err: %s", err)
		}

		if err := conn.DeleteBucket(bucket); err != nil {
			t.Fatalf("err: %s", err)
		}
	}()
