...
func setupCassandraTLS(conf map[string]string, cluster *gocql.ClusterConfig) error {
	...
	if pemBundlePath, ok := conf["pem_bundle_file"]; ok {
		...
		pemJSON, err := certutil.ParsePKIJSON(pemJSONData)
		if err != nil {
			return err
		}
	...
	if tlsSkipVerifyStr, ok := conf["tls_skip_verify"]; ok {
		...
		tlsConfig.InsecureSkipVerify = tlsSkipVerify != 0
	}
...
// buckets returns all the prefix buckets the key should be stored at
...
func (c *CassandraBackend) Put(ctx context.Context, entry *physical.Entry) error {
	...
	for i, _bucket := range buckets {
		go func(i int, bucket string) {
			...
			results <- c.sess.Query(stmt, bucket, entry.Key, value).WithContext(ctx).Exec()
		}(i, _bucket)
	}
	for range buckets {
...
func (c *CassandraBackend) Get(ctx context.Context, key string) (*physical.Entry, error) {
	...
	q := c.sess.Query(stmt, c.bucket(key), key).WithContext(ctx)
...
func (c *CassandraBackend) Delete(ctx context.Context, key string) error {
	...
	for _, bucket := range buckets {
		go func(bucket string) {
			results <- c.sess.Query(stmt, bucket, key).WithContext(ctx).Exec()
		}(bucket)
	}
...
// deduplicated. It is the shared implementation behind List and ListPage;
// Cassandra has no native way to paginate a range query mid-stream once the
// results have been folded down to folder components, so ListPage pages
// over the fully materialized (but already-deduplicated) slice in Go.
func (c *CassandraBackend) listAll(ctx context.Context, prefix string) ([]string, error) {
	defer metrics.MeasureSince([]string{"cassandra", "list"}, time.Now())

	stmt := fmt.Sprintf(`SELECT key FROM "%s" WHERE bucket = ?`, c.table)
	q := c.sess.Query(stmt, c.bucketName(prefix)).WithContext(ctx)
	iter := q.Iter()
	k, keys := "", []string{}
	for iter.Scan(&k) {
...
func (c *CassandraBackend) ListPage(ctx context.Context, prefix, after string, limit int) ([]string, error) {
