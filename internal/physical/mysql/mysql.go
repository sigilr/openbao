...
func NewMySQLBackend(conf map[string]string, logger log.Logger) (physical.Backend, error) {
	...
	if !tableExist {
		createQuery := "CREATE TABLE IF NOT EXISTS " + dbTable +
			" (bao_key varbinary(3072), bao_value mediumblob, PRIMARY KEY (bao_key))"
	...
	if !ok {
		locktable = "openbao_ha_locks"
	}

	if err := validate(locktable); err != nil {
		return nil, fmt.Errorf("invalid lock_table: %w", err)
	}
	...
	if haEnabled {
		...
		if err := lockTableRows.Close(); err != nil {
			return nil, fmt.Errorf("failed to check mysql table exist: %w", err)
		}

		// Create the required table if it doesn't exists.
		if !lockTableExist {
			createQuery := "CREATE TABLE IF NOT EXISTS " + dbLockTable +
	...
	statements := map[string]string{
		"put": "INSERT INTO " + dbTable +
			" VALUES( ?, ? ) ON DUPLICATE KEY UPDATE bao_value=VALUES(bao_value)",
		"get":    "SELECT bao_value FROM " + dbTable + " WHERE bao_key = ?",
		"delete": "DELETE FROM " + dbTable + " WHERE bao_key = ?",
		"list":   "SELECT bao_key FROM " + dbTable + " WHERE bao_key LIKE ? ORDER BY bao_key ASC",
	}
...
func NewMySQLClient(conf map[string]string, logger log.Logger) (*sql.DB, error) {
	...
	// Create MySQL handle for the database, via the mysql driver's own DSN
	// builder rather than hand-rolled string concatenation.
	cfg := mysql.NewConfig()
	cfg.User = username
	cfg.Passwd = password
	cfg.Net = "tcp"
	cfg.Addr = address
	cfg.Params = dsnParams
...
func (m *MySQLBackend) Put(ctx context.Context, entry *physical.Entry) error {
	...
	_, err := m.statements["put"].ExecContext(ctx, entry.Key, entry.Value)
	return err
}
...
// listAll returns the sorted, deduplicated set of immediate children of
// prefix: entries are truncated at the next "/", matching the hierarchical
// listing semantics of physical.Backend. The underlying query already
// returns rows in bao_key order, so the result only needs a final stable
// sort to account for any ties introduced by truncation.
func (m *MySQLBackend) listAll(ctx context.Context, prefix string) ([]string, error) {
...
// ListPage is used to list all the keys under a given prefix, in sorted
// order, starting strictly after the given "after" cursor, up to "limit"
// entries (non-positive limit means unlimited). This backend has no native
// range-query support over its flattened key table, so the page is derived
// from the same fully-materialized, sorted key set as List; the sort and
// dedup work is shared via listAll.
...
func (i *MySQLHALock) hasLock(key string) error {
	...
	if errors.Is(err, sql.ErrNoRows) || !result.Valid {
...
var (
	...
	// ErrClaimFailed is returned if the leader information couldn't be updated in the DB.
	ErrClaimFailed = errors.New("mysql: unable to update DB with new leader information")
	// ErrSettingGlobalID is returned if between getting the lock and checking the ID of it we lost it.
...
// becomeLeader updates the current cluster leader in the DB. This is used so
...
func (i *MySQLLock) Lock() error {
	...
	const lockTimeoutSeconds = 1<<31 - 1

	rows, err := i.in.Query("SELECT GET_LOCK(?, ?), IS_USED_LOCK(?)", i.key, lockTimeoutSeconds, i.key)
	if err != nil {
		return err
	}
	defer rows.Close() //nolint:errcheck
	...
	if err := rows.Scan(&lock, &connectionID); err != nil {
		return err
	}
...
// do it from the same mysql connection_id that you originally created it from,
// which is a much larger hassle to do reliably; closing the connection instead
// ensures we don't ever get into a state where releasing the lock hangs, and is
// much less code.
...
// setupMySQLTLSConfig establishes a TLS connection with a given CA certificate.
// It registers a tls.Config associated with the same key as the "tls" DSN
// parameter, e.g. foo:bar@tcp(127.0.0.1:3306)/dbname?tls=default
func setupMySQLTLSConfig(tlsCaFile string) error {
	...
	if ok := rootCertPool.AppendCertsFromPEM(pem); !ok {
		return errors.New("failed to parse tls_ca_file as PEM")
	}

	return mysql.RegisterTLSConfig(mysqlTLSKey, &tls.Config{
