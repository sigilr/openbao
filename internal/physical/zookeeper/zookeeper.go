...
// ZooKeeperBackend is a physical backend that stores data at a specific
// prefix within ZooKeeper. It is used in production situations, as it
// allows OpenBao to run on multiple machines in a highly-available manner.
...
// NewZooKeeperBackend constructs a ZooKeeper backend using the given API
// client and the prefix in the KV store.
func NewZooKeeperBackend(conf map[string]string, logger log.Logger) (physical.Backend, error) {
	...
	if !ok {
		...
		// (e.g. ':MyUser' which omits the schema, or ':' omitting both).
		if owner == "" || schema == "" {
	...
	if useAddAuth {
		if err := client.AddAuth(schema, []byte(owner)); err != nil {
...
// createClient returns a client for a ZK connection. The config value
// 'tls_enabled' determines whether TLS is enabled or not.
func createClient(conf map[string]string, machines string, timeout time.Duration) (*zk.Conn, <-chan zk.Event, error) {
	...
	if isTLSEnabled {
		// Create a custom Dialer with cert configuration for the TLS handshake.
...
// customTLSDial supports the following configuration properties:
...
//  6. tls_verify_ip: if set to true, the server's IP is verified in the
//     certificate when tls_skip_verify is false.
func customTLSDial(conf map[string]string, machines string) zk.Dialer {
	return func(network, addr string, timeout time.Duration) (net.Conn, error) {
		...
		if sParseErr != nil {
			// If the address is only missing a port, assign the full address anyway.
		...
		if !insecureSkipVerify {
			// If tls_verify_ip is set to false, the server's DNS name is verified against
			// the CN/SAN of the certificate. If tls_verify_ip is true, the server's IP is
			// verified instead. These checks happen only when tls_skip_verify is false.
			...
			// Here we look up the DNS name and assign it to serverName if ipSanCheck is false.
			if !ipSanCheck {
				...
				if lookupErr == nil {
					for _, lookupAddress := range lookupAddressMany {
						...
						lookupAddress = strings.TrimSuffix(lookupAddress, ".")
						// Allow serverName to be replaced only if the lookup name is part of the
						// supplied machine names. If there is no match, the serverName will
						// continue to be an IP value.
		...
		if err := conn.Handshake(); err != nil {
			return nil, fmt.Errorf("handshake failed with ZooKeeper: %w", err)
		}
		return conn, nil
	}
}
...
func (c *ZooKeeperBackend) ensurePath(path string, value []byte) error {
	...
	for index, node := range nodes {
		...
		exists, _, _ := c.client.Exists(fullPath)
		switch {
		case !isLastNode && !exists:
...
// cleanupLogicalPath is used to remove all empty nodes, beginning with the
// deepest one, aborting on the first non-empty one, up to the top-level node.
func (c *ZooKeeperBackend) cleanupLogicalPath(path string) error {
	...
	for i := len(nodes) - 1; i > 0; i-- {
		...
		switch {
		case stat.DataLength > 0 && stat.NumChildren > 0:
			panic(fmt.Sprintf("node %q is both of data and leaf type", fullPath))
		case stat.DataLength > 0:
		...
		case stat.NumChildren > 0:
			return nil
		default:
			// Empty node, let's clean it up!
			if err := c.client.Delete(fullPath, -1); err != nil && !errors.Is(err, zk.ErrNoNode) {
...
// nodePath returns a zk path based on the given key.
...
// listAll returns the sorted set of children directly under prefix, with a
// trailing slash for "folders". It is shared by List and ListPage.
func (c *ZooKeeperBackend) listAll(ctx context.Context, prefix string) ([]string, error) {
	...
	// If the path nodes are missing, there are no children!
	if errors.Is(err, zk.ErrNoNode) {
		return []string{}, nil
	} else if err != nil {
		return []string{}, err
	...
	for _, key := range result {
		...
		if err != nil {
			// The node ought to exist, so it must be something else that's wrong.
			return []string{}, err
		}

		// Check if this entry is a leaf of a node, and append the slash which is
		// what OpenBao depends on for iteration.
		switch {
		case stat.DataLength > 0 && stat.NumChildren > 0:
			if childPath == c.nodePath("core/lock") {
				// go-zookeeper Lock() breaks OpenBao semantics and creates a directory
				// under the lock file; just treat it like the file OpenBao expects.
				children = append(children, key[1:])
			} else {
				panic(fmt.Sprintf("node %q is both of data and leaf type", childPath))
			}
		case stat.DataLength == 0:
			// We cannot differentiate here based on the number of children, as a node
			// can have all its leaves removed and still be a node.
...
func (c *ZooKeeperBackend) List(ctx context.Context, prefix string) ([]string, error) {
	return c.listAll(ctx, prefix)
}

// ListPage is used to list a page of keys under a given prefix, starting
// strictly after the "after" key, up to "limit" keys.
func (c *ZooKeeperBackend) ListPage(ctx context.Context, prefix, after string, limit int) ([]string, error) {
	keys, err := c.listAll(ctx, prefix)
	if err != nil {
		return nil, err
	}
...
func (i *ZooKeeperHALock) unlockInternal() error {
	...
	if err := i.zkLock.Unlock(); err != nil {
		return err
	}
