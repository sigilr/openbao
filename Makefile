...
memcached-database-plugin:
	@CGO_ENABLED=0 go build -o bin/memcached-database-plugin ./internal/builtin/database/memcached/memcached-database-plugin

.PHONY: bin default prep test vet bootstrap fmt fmtcheck memcached-database-plugin ember-dist ember-dist-dev static-dist static-dist-dev assetcheck check-openbao-in-path packages build build-ci semgrep semgrep-ci vet-godoctests ci-vet-godoctests
