...
- **`runner/`** — spoke-side dispatcher. Holds the per-`instance_id` plugin cache (single-flighted on cold-miss, refcounted while in use, idle-evicted after 24h) and dispatches incoming JSON requests to the in-process built-in database plugins.
...
- **`command/agent_*.go`** — the operator CLI: `bao agent init | join | run | list | renew | ca status | ca rotate | token create | list | revoke`. `bao agent run` is the long-running spoke daemon and ships inside the same `bao` binary. The agent backend also exposes `agent/ca/update-endpoint` for non-destructive endpoint/SAN refresh — invoked via raw `bao write`, not a dedicated subcommand.
