...
description: >-
  The PostgreSQL plugin for OpenBao's database secrets engine generates database
...
- `tls_ca` `(string: "")` - Specifies the inline PEM contents of a CA certificate
  bundle used to verify the PostgreSQL server certificate. Use a verifying
  `sslmode`, such as `verify-full`, in `connection_url`.

- `tls_certificate` `(string: "")` - Specifies the inline PEM contents of the
  client certificate used for PostgreSQL TLS authentication. Must be provided
  together with `tls_key`.

- `tls_key` `(string: "")` - Specifies the inline PEM contents of the
  private key corresponding to `tls_certificate`. Must be provided together
  with `tls_certificate`.
...
- `password_authentication` `(string: "password")` - When set to "scram-sha-256", passwords will be hashed by OpenBao and stored as-is by PostgreSQL.
